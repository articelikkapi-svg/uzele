...
// In production we don't load the create-platform dev helpers.
// Provide a minimal SessionProvider fallback so the app builds as static.
const SessionProvider: React.FC<{ children?: React.ReactNode }> = ({ children }) => <>{children}</>;
import { useNavigate } from 'react-router';
import { serializeError } from 'serialize-error';
import { Toaster } from 'sonner';
// Dev-only virtual fonts loader replaced with a no-op for production builds
const LoadFonts: React.FC = () => null;
import type { Route } from './+types/root';
// Dev helpers (removed in production build)

export const links = () => [];

// No global fetch override in production build.
...
const useCodeGen = () => {
  // No-op in production; dev-only codegen integrations removed.
  return;
};
...
export function Layout({ children }: { children: ReactNode }) {
  // Dev-only integrations are skipped in production.
  ...
  return (
    <html lang="en">
      <head>
        ...
        <Meta />
        <Links />
        {/* Dev overlay removed for production */}
